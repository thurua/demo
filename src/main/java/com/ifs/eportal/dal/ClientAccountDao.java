...
public class ClientAccountDao implements Repository<ClientAccount, Integer> {
	...
	public int updateAutoRequest(boolean autoRequest, String sfId) {
		...
		try {
			String sql = ZFile.read(_path + "update_auto_request__c.sql");

			// Execute
			Query q = _em.createNativeQuery(sql);
			q.setParameter("autoRequest", autoRequest);
			q.setParameter("sfId", sfId);
			res = q.executeUpdate();
		} catch (Exception ex) {
			if (ZConfig._printTrace) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public LoanDetailDto getLoanDetailBy(String sfId) {
	...
	/**
	 * Get factoring detail by
	 * 
	 * @param sfId
	 * @return
	 */
	@SuppressWarnings("unchecked")
	public FactoringDetailDto getFactoringDetailBy(String sfId) {
		...
		try {
			...
			// Convert
			res = FactoringDetailDto.convert(t, l);
		} catch (Exception ex) {
			if (ZConfig._printTrace) {
				ex.printStackTrace();
			}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public LctrDetailDto getLctrDetailBy(String sfId) {
